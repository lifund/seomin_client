...
app.get('/home',(req,res)=>{
	...
	mongoFind('menu',{"mainPage": true },(result)=>{
		...
		// make banner slider
		let bannerHTML = ''
		fs.readdirSync(path.join(__dirname,'public/banner'),'utf-8').forEach((bannerImage)=>{
			bannerHTML += '<img src="public/banner/'+bannerImage+'">';
		})
		...
		result.forEach((menu)=>{
			menuCardHTML += `
			<a class="menuCard" data-all="${encodeURIComponent(JSON.stringify(menu))}">
				<img src="public/menuImage${menu.imageURL}" alt="이미지 준비중">
				<p> ${menu.productName} </p>`
			menu.price.forEach((priceTag)=>{
				menuCardHTML+=`
				<div class="menuCard_price_size">${priceTag.size}</div>
				<div class="menuCard_price_value">${priceTag.value}</div>`
			})
			menuCardHTML+=`
			</a>`
		})
...
app.get('/brand',(req,res)=>{
	res.sendFile(path.join(__dirname,'public/client/brand.html'));
});

//-------------------- MENU --------------------//
app.get('/menu',(req,res)=>{
	mongoFind('menu',{"categoryName": "메인메뉴" },(result)=>{
		...
		let targetJson = {
			...
			"contents":
			{
				"menu_container": {"kor": menuCardHTML}
			}
		}
		const mulang_home = new mulang({
			sourceHtml: sourceHtml,
	...
	mongoFind('shop',{},(result)=>{
		...
		result.forEach((shop)=>{
			...
			if(region[shop.city]){
				...
				region[shop.city] = {"count":1,"district":{}}
			}
		...
		let targetJson = {
			...
			"contents":
			{
				...
				"region_data": {"kor": encodeURIComponent(JSON.stringify(region))},
			}
		}
		const mulang_home = new mulang({
			sourceHtml: sourceHtml,
...
app.get('/shop/inquiry',(req,res)=>{
	res.sendFile(path.join(__dirname,'public/client/shop_inquiry.html'));
});
...
app.get('/franchise',(req,res)=>{
	res.sendFile(path.join(__dirname,'public/client/franchise.html'));
});
app.get('/franchise/inquiry',(req,res)=>{
	res.sendFile(path.join(__dirname,'public/client/franchise_inquiry.html'));
});
